...
const RedisPresence_1 = require("./presence/RedisPresence");
const MongooseDriver_1 = require("./matchmaker/drivers/MongooseDriver");
...
function handleCreateRoom(roomName, clientOptions) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (exports.presence instanceof RedisPresence_1.RedisPresence && clientOptions.roomId !== undefined) {
            var redisPresence = exports.presence;
            var uniqueRoomId = clientOptions.roomId;
            console.log(`Awaiting redis lock for ${uniqueRoomId}`);
            lock = yield redisPresence.redlock.acquire(uniqueRoomId, 1000).catch(error => {
                console.log(`Redis lock for ${uniqueRoomId} expired after 1 second. with error: ${error}`);
                throw new ServerError_1.ServerError(Protocol_1.ErrorCode.MATCHMAKE_LOCK_EXPIRE, `redis lock for ${uniqueRoomId} expired`);
            });
            console.log(`Redis lock for ${uniqueRoomId} acquired.`);
            // Check if room exists post lock acquisition.
            var existingRoom = yield exports.driver.findOne({ roomId: uniqueRoomId });
